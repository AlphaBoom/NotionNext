...
对于低流量博客足够。一次游戏只在开始和手动上传时写库；排行榜在展开、手动刷新和上传成功时读取前 20 名。客户端读取使用 `cache: 'no-store'`，避免上传后刷新恢复旧榜单。D1 有排序索引，避免为展示前 20 名扫描整个排行榜。限流也占少量数据库读写；额度按账号共享，需计入已有 Worker 的用量。
...
- 开局创建随机凭证，死亡结算界面提供“上传成绩 / 查看排名”按钮，点击后在弹窗填写昵称、手动上传；不自动公开成绩。Esc 优先关闭排行榜，再次按下才退出游戏。
