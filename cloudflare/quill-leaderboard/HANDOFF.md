...
用户在预览完成游戏后没有察觉排行榜。此前入口仅在无限模式画布下方，首次关卡没有提示，功能链路通过并不代表入口容易发现。本次调整：

- 游戏标题旁常驻“排行榜 TOP 20”，首次关卡和无限模式均可查看。点击在当前视口打开原生模态弹窗，不需要滚动到画布下面。
- 无限模式死亡结算直接显示“上传成绩 / 查看排名”按钮，打开后聚焦昵称输入框；排行榜自动展开并刷新。关闭重开保留本局提交状态，不重复上传。
- 首次关卡明确提示解锁条件，通关提示再次点击头像参加无限挑战；首次关卡依然不上传成绩。
- 查看榜单暂停进行中的游戏；弹窗内的 P、方向键等不控制游戏。Esc 优先关闭弹窗，再次按下才退出游戏。
- 相关 Jest 四组 19 项通过。游戏生命周期测试现在使用实际排行榜组件，覆盖结算按钮打开表单、昵称聚焦和上传冻结成绩；组件测试补充关闭重开保留已上传状态。
- 浏览器验证首屏入口、居中榜单、首次关卡查看规则、查看时暂停、P 不恢复和 Esc 只关弹窗通过。本次没有重新完成一局自然死亡的浏览器验收；该流程的前次验证见上节，本次结算接入以组件集成测试覆盖。

## 同步游戏优化与结算改版（2026-09-10）

- 先合入主分支 `95bbe3a`（PR #21）。保留经验结算、待选升级队列、8 槽构筑、进化、精英和重抽逻辑；测试清理使用主分支的 `react/pure` 与 DOM 存在性检查。补充隐藏页面重新开局的衔接：恢复可见后才申请新局凭证，避免重开时丢失凭证或沿用上一局。
- 死亡后自动读取榜单，在游戏内成绩卡直接显示本局数据和预计名次。昵称、上传、放弃上传继续挑战、返回博客及榜单详情均在结算处可操作，不再必须先打开弹窗。
- 排名预估仅比较公开前 20 名，平局排在已有成绩之后；榜外显示 `20+ / 暂未进入前 20 名`。未提交时明确标记预计，提交后采用服务器的最佳纪录排名。较低成绩保留旧最佳，页面对应显示“最佳纪录排名”。
- 弹窗重做为主题一致的“夜行榜”，全部使用现有主题变量；前三名、本人标记、空榜提示和刷新操作重新排版。客户端没有增加运行时依赖，Worker、数据库 schema 及生产配置无需变更。
- 57 个游戏/前端测试与 7 个 Worker/SQLite 测试通过，包含主分支新增成长测试、平局与前 20 边界、仅预估不上传、异步读取不能覆盖提交结果、关闭详情保留结果及隐藏重开凭证回归。
- 浏览器使用隔离 QA 构建的可控结算样例（12.345 秒 / 击退 20 / 首领 0 / LV.5），检查实际组件和本地原版 Worker/SQLite：结算自动预估第 2，查看详情前没有写入成绩；模拟响应丢失、重试后保留更好的旧纪录并显示最佳排名第 1，榜单高亮本人，成绩条数不增加。普通、深色、NEW GAME! 三套主题颜色和结算/详情布局均检查。样例结束开关只存在临时 QA 构建，未进入仓库或生产；本次不是自然死亡通关验收。
