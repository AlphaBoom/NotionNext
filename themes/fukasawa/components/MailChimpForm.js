...
export default function MailChimpForm() {
  ...
  return <>
        {siteConfig('FUKASAWA_MAILCHIMP_FORM', null, CONFIG) && <div className="sm:col-span-6 md:col-span-3 lg:col-span-3">
            ...
            <form onSubmit={handleSubmit} aria-busy={status === 'pending'}>
                <div className="flex flex-wrap mb-4">
                    <div className="w-full">
                        <label className="block text-sm sr-only" htmlFor="newsletter">{locale.MAILCHIMP.EMAIL}</label>
                        <div className="relative flex items-center max-w-xs">
                            <input name="email" disabled={disabled} id="newsletter" type="email" className="form-input w-full text-gray-800 px-3 py-2 pr-12 text-sm" placeholder={locale.MAILCHIMP.EMAIL} required />
                            <button disabled={disabled} type="submit" className="absolute inset-0 left-auto" aria-label="Subscribe">
                        ...
                        {status === 'error' && <p role="alert" className="mt-2 text-red-600 text-sm">Subscription failed. Please try again.</p>}
                    </div>
                </div>
            </form>
        </div>
