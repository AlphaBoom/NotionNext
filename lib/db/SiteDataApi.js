...
import { isDatabaseEntryPage } from './notion/pageMetadata'
...
import { collectionIdFor } from '@/lib/db/notion/database/model'
...
// Invalidate derived data and its root schema together when the post shape changes.
...
function getGlobalDataCacheKey({ pageId, locale }) {
  ...
  return `global_data_${SITE_DATA_CACHE_VERSION}_${safeLocale}_${safePageId}`
}
...
export async function getSiteDataByPageId({ pageId, from }) {
  ...
  const cacheKey = `site_${SITE_DATA_CACHE_VERSION}_${pageId}`
...
function pruneUnusedCollectionRecords(blockMap) {
  ...
  Object.values(blockMap.block).forEach(entry => {
    ...
    const collectionId = collectionIdFor(block, blockMap)
...
export async function resolvePostProps({ prefix, slug, suffix, locale, from }) {
  ...
  const findPost = () => {
    ...
    return (
      ...
      // 2. UUID 匹配
      props.allPages.find(p =>
        String(p?.id || '').replace(/-/g, '').toLowerCase() ===
        fullSlug.replace(/-/g, '').toLowerCase()
      ) ||
      null
    )
  }
  ...
  post = findPost()
  // allPages already contains only records from the publishing database.
  const isPublishingPage = Boolean(post)
  ...
  if (post) {
    ...
    post.noIndex = isPublishingPage
      ? false
      : await isDatabaseEntryPage({
        pageId: post.id,
        recordMap: post.blockMap,
        publishingPageIds: (props.allPages || []).map(page => page.id),
        loadBlock: async id => {
          const blocks = await fetchInBatches([id])
          return adapterNotionBlockMap({ block: blocks })?.block?.[id]?.value
        }
      })
    props.post = post
...
function cleanBlock(item) {
  ...
  if (pageBlock) {
    for (const i in pageBlock) {
      ...
      // Auxiliary entry pages render their database properties in the page header.
      if (!post.noIndex) {
