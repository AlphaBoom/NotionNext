...
export async function fetchConfigPageData(configPageId) {
  let pageRecordMap = await fetchNotionPageBlocks(configPageId, 'config-table', { fetchCollections: true })
  ...
  for (const table of ['Config-Table', 'CONFIG-TABLE']) {
    if (content) break
    pageRecordMap = await fetchNotionPageBlocks(configPageId, table, { fetchCollections: true })
