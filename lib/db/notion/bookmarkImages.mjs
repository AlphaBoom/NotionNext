...
export function installBookmarkImageFallback(root, notionHost) {
  ...
  const recover = image => {
    ...
    const slot = /** @type {HTMLElement | null} */ (image.closest('.notion-bookmark-image, .notion-bookmark-link-icon'))
    if (slot) slot.style.display = 'none'
    else image.style.display = 'none'
  }
  /** @param {Event} event */
  const onError = event => recover(/** @type {HTMLImageElement | null} */ (event.target))
  root.addEventListener('error', onError, true)
  // An image may already have failed before hydration attached the listener.
  const images = /** @type {NodeListOf<HTMLImageElement>} */ (root.querySelectorAll('.notion-bookmark img'))
