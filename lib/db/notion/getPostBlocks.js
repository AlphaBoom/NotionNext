...
import { prunePageScope } from '@/lib/db/notion/database/pageScope'
import { hydrateDatabaseMetadata } from '@/lib/db/notion/database/hydrateMetadata'
...
export function getPageBlockCacheKey(id, cacheVersion, fetchCollections = false) {
  const normalizedVersion = normalizePageBlockCacheVersion(cacheVersion)
  // Refresh content previously pruned without synced blocks or alias targets.
  const scope = fetchCollections ? 'v2_index' : 'v3_content'
  const key = `page_block_${scope}_${id}`
  return normalizedVersion ? `${key}_${normalizedVersion}` : key
}

/**
 * @param {string} id
 * @param {string | undefined} from
 * @param {{ cacheVersion?: string | number | Date, fetchCollections?: boolean }} options
 */
export async function fetchNotionPageBlocks(id, from = undefined, options = {}) {
  const cacheKey = getPageBlockCacheKey(id, options?.cacheVersion, options.fetchCollections)

  const pageBlock = await getOrSetDataWithCache(
    cacheKey,
    () => limit(() => getPageWithRetry(id, from, 3, cacheKey, options))
  )
  ...
  if (await hydrateExternalObjectInstances(pageBlock)) {
...
/**
 * react-notion-x renders inline external mentions (for example GitHub links)
 * from an `eoi` rich-text decorator plus a referenced block. notion-client's
 * missing-block pass only follows block.content, so these rich-text-only block
 * references need to be fetched separately.
 */
export function getMissingExternalObjectInstanceIds(recordMap) {
  const ids = new Set()

  const visit = value => {
    if (Array.isArray(value)) {
      if (value[0] === 'eoi' && typeof value[1] === 'string' && value[1]) {
        ids.add(value[1])
      }
  ...
  return [...ids].filter(id => !recordMap.block[id])
}

export async function hydrateExternalObjectInstances(
  recordMap,
  fetchBlocks = fetchInBatches
) {
  const missingIds = getMissingExternalObjectInstanceIds(recordMap)
  if (!missingIds.length) return false

  const blocks = await fetchBlocks(missingIds)
...
export async function getPageWithRetry(
  ...
  try {
    ...
    const raw = await notionAPI.getPage(id, { fetchCollections: options.fetchCollections === true })
    const pageData = options.fetchCollections ? raw : prunePageScope(raw, id)
    if (!options.fetchCollections) {
      await hydrateDatabaseMetadata(pageData, requests => notionAPI.__call('fetch', {
        endpoint: 'syncRecordValuesMain', body: { requests }
      }))
    }
    ...
    return getPageWithRetry(id, from, retryAttempts - 1, cacheKey, options)
  }
}
