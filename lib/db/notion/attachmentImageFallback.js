import { isNotionImageSource } from '@/lib/utils/notion.util'

/** Retry a failed Notion upload proxy once through the stable original-file entry. */
export function installAttachmentImageFallback(root, notionHost) {
  ...
  const recover = image => {
    ...
    try {
      ...
      if (!isNotionImageSource(source) || !id) return
