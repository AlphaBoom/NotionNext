...
export function adapterNotionBlockMap(blockMap) {
  ...
  if (blockMap.signed_urls) {
    // These images already render through mapImgUrl's stable attachment proxy.
...
/** Share the renderer's attachment fallback with page-data normalization. */
export function getStableImageSource(imageUrl, block) {
  ...
  if (
    ...
    (
      ...
      (typeof imageUrl === 'string' && imageUrl.startsWith(`${attachment}?`))
    )
  ) {
    return attachment
  }
