...
export function prunePageScope(recordMap, rootId) {
  ...
  const visit = id => {
    // A mention can retain metadata before the same block is reached as content.
    ...
    if (id !== root && ['page', 'collection_view_page'].includes(block.type))
      return
    // Linked pages need titles/icons only; synced blocks render their target body.
    const aliasId = block.format?.alias_pointer?.id
    if (typeof aliasId === 'string') keep.add(aliasId)
    const syncedId = block.format?.transclusion_reference_pointer?.id
    if (typeof syncedId === 'string') visit(syncedId)
