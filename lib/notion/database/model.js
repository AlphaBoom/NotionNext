...
// Translate visitor controls to the same filter shape used by public Notion views.
export function publicViewQuery(view, query, schema) {
  const filters = []
  ...
  for (const item of query.filters) {
    ...
    const operator = publicFilterOperator(type, item.operator)
    const date = ['date', 'created_time', 'last_edited_time'].includes(type)
    filters.push({
      ...
      filter: {
        ...
        ...(!['is_empty', 'is_not_empty'].includes(operator) && {
          value: {
            type: 'exact',
            value: date ? { type: 'date', start_date: item.value } : item.value
          }
        })
      }
    })
  ...
  return {
    ...
    sort: query.sorts.length ? query.sorts : view.query2?.sort || [],
    searchQuery: query.search
  }
}

function publicFilterOperator(type, operator) {
  if (['is_empty', 'is_not_empty'].includes(operator)) return operator
  const compare = { equals: 'is', does_not_equal: 'is_not' }
  if (type === 'number') return `number_${operator}`
  if (type === 'checkbox') return `checkbox_${compare[operator]}`
  if (['select', 'status'].includes(type)) return `enum_${compare[operator]}`
  if (type === 'multi_select') return `enum_${operator}`
  if (['date', 'created_time', 'last_edited_time'].includes(type))
    return `date_${operator === 'equals' ? 'is' : `is_${operator}`}`
  return `string_${compare[operator] || operator}`
}
