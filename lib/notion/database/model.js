...
export function savedViewQuery(view) {
  ...
  return {
    ...(filters.length
      ? { filter: { operator: view.filter_operator || 'and', filters } }
      : {}),
    sort: Array.isArray(sorts)
      ? sorts
          .filter(sort => sort?.property || sort?.property_id)
          .map(sort => ({
            property: sort.property || sort.property_id,
