...
test('Notion selects and cached values give the same distinct disclosures', () => {
  for (const [label, mode, description] of [
    ['AI 润色', 'ai-polished', '本文由作者撰写，AI 辅助润色。'],
    ['AI 生成', 'ai-generated', '本文由 AI 生成。']
  ]) {
...
test('feed readers see authorship before summary or full content without changing the post', () => {
  ...
  assert.equal(getWritingModeSummary(post), '[AI 生成] 本文由 AI 生成。 原摘要')
  assert.equal(prependWritingModeNotice('<h2>正文</h2>', post.writingMode), '<p>本文由 AI 生成。</p><h2>正文</h2>')
...
test('empty summaries and unavailable full content still disclose AI involvement', () => {
  assert.equal(getWritingModeSummary({ writingMode: 'ai-polished' }), '[AI 润色] 本文由作者撰写，AI 辅助润色。')
  assert.equal(prependWritingModeNotice(undefined, 'ai-generated'), '<p>本文由 AI 生成。</p>')
})
